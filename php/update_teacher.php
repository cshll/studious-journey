...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['id'], $_POST['full_name'], $_POST['address'], $_POST['email'], $_POST['phone_number'])) {
  ...
  $teacher_stmt = $pdo->prepare("SELECT COUNT(*) FROM teachers WHERE teacher_id = :teacher_id");
  ...
  // Check if email is valid after sanitized.
  $email = filter_var(trim($_POST['email']), FILTER_SANITIZE_EMAIL);
  if (!filter_var($email, FILTER_VALIDATE_EMAIL) || strlen($email) > 100) {
    die("Invalid email provided.");
  }

  // Check if phone number matches regular expression (UK only). 
  ...
  try {
    // Update teacher on the database.
